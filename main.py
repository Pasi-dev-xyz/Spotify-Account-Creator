...
class Main:
    ...
    def __init__(self):
        init(convert=True)
        self.SetTitle('[Spotify Account Creator Tool]')
        self.clear()
    ...
    def TitleUpdate(self):
        while True:
            self.SetTitle(f'[Spotify Account Creator] ^| CREATED: {self.createds} ^| WEBHOOK RETRIES: {self.webhook_retries} ^| RETRIES: {self.retries} ^| THREADS: {active_count()-1}')
            sleep(0.1)
    ...
    def SendWebhook(self,title,message,icon_url,thumbnail_url,proxy,useragent):
        try:
            ...
            message_to_send = {"embeds": [{"title": title,"description": message,"color": 65362,"author": {"name": " [CLICK HERE]","url": "https://pasi-dev.xyz","icon_url": icon_url},"footer": {"text": "MADE BY Pasi","icon_url": icon_url},"thumbnail": {"url": thumbnail_url},"timestamp": timestamp}]}
                ...
                if json_data['status'] == 1:
                    ...
                    if username != '':
                        ...
                        if self.webhook_enable == 1:
                            self.SendWebhook('Spotify Account','**COMBO**\n{0}:{1}\n**USERNAME**\n{2}\n**GENDER**\n{3}\n**BIRTHDATE**\n{4}/{5}/{6}'.format(credentails['email'],credentails['password'],credentails['username'],credentails['gender'],credentails['birth_year'],credentails['birth_month'],credentails['birth_day']),'','https://upload.wikimedia.org/wikipedia/commons/thumb/1/19/Spotify_logo_without_text.svg/768px-Spotify_logo_without_text.svg.png',proxy,self.GetRandomUserAgent())
                    else:
                        self.retries += 1
                        self.SpotifyCreator()
                else:
